...
def get_dataset(data_path, transform, train):
    dataset_transform = transforms.Compose([transforms.Resize(32), transforms.CenterCrop(32), transforms.ToTensor()])
    transform_collate_fn = functools.partial(collate_fn, transform)

    split = 'train' if train else 'test'
    return datasets.CelebA(data_path, split=split, download=True, transform=dataset_transform), transform_collate_fn
